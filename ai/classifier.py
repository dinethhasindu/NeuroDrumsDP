...
class DrumClassifier:
    """Uses a supplied trained RF model when present; otherwise a deterministic multi-feature AI fallback."""
    def __init__(self, model_path='models/drum_rf_classifier.pkl'):
        self.rf=None; self.scaler=None; self.mode='Signal AI'
        if os.path.isfile(model_path):
            try:
                with open(model_path,'rb') as f: p=pickle.load(f)
                self.rf=p.get('model'); self.scaler=p.get('scaler'); self.mode='RandomForest AI'
            except Exception: pass
    def classify(self, f):
        if self.rf is not None:
            try:
                v=feature_vector(f).reshape(1,-1)
                if self.scaler is not None: v=self.scaler.transform(v)
                probs=self.rf.predict_proba(v)[0]
                names=list(self.rf.classes_)
                d={n:0.0 for n in LANE_NAMES}
                for n,p in zip(names,probs):
                    if n in d: d[n]=float(p)
                k=max(d,key=d.get); return k,d[k],d
            except Exception: pass
        low,mid,high,sub=f['low_energy'],f['mid_energy'],f['high_energy'],f['subbass_energy']
        c=f['spectral_centroid']; z=f['zcr']; dur=f['duration']; flat=f['spectral_flatness']
        ...
        scores['Closed Hat']=0.15+1.9*high+max(0,c/9000)
        ...
        if max(scores.values())<0.65: scores['FX']+=0.2
        total=sum(max(0,v) for v in scores.values()); probs={k:max(0,v)/total for k,v in scores.items()}
        k=max(probs,key=probs.get); return k,probs[k],probs
