    ...
    def _run(self,path,sensitivity,use_gpu,skip,cb,done):
        try:
            ...
            if skip:
                ...
                drum=separate_drums(path,use_gpu,cb=lambda f:self._p(cb,1,STAGES[1],f))
                self._p(cb,1,STAGES[1],1)
            ...
            for i,(t,f) in enumerate(zip(times,feats)):
                ...
                e=DrumEvent(type=typ,start=float(t),end=float(t+dur),duration=dur,velocity=float(np.clip(f['rms']*5,0.15,1)),confidence=float(conf),uncertain=conf<0.45,low_energy=f['low_energy'],mid_energy=f['mid_energy'],high_energy=f['high_energy'],spectral_centroid=f['spectral_centroid'],zcr=f['zcr'],onset_strength=float(f.get('onset_strength_peak',0)))
                events.append(e); self._p(cb,4,STAGES[4],(i+1)/max(1,len(times)))
            events=group_rolls(events); self._p(cb,5,STAGES[5],1)
            if done: done(True,'',events,info,bpm,self.classifier.mode)
        except Exception as exc:
            if done: done(False,str(exc),[],AudioInfo(path=path,filename=os.path.basename(path)),120.0,self.classifier.mode)
